...
public class UserEnterApplication {
	...
	public static void main(String[] args) {
		SpringApplication.run(UserEnterApplication.class, args);
		}
	...
	public Thread workingThread() {
		Thread worker = new Thread(() -> {
			while (true) {
				String message = redisMQ.messagePop(DouyuMessageType.uenter);
				Map<String, String> map = parseMessage(message);
				if (map != null) {
					UserEnter userEnter = UserEnter.getInstance(map);
					audienceRedisRepository.userEnter(userEnter);
					String display = userEnter.toString();
