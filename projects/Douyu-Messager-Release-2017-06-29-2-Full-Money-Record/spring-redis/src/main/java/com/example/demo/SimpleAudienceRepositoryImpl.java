...
public class SimpleAudienceRepositoryImpl implements AudienceRepository {
	...
	public long moneySum(){
		Set<String> keys = redisTemplate.keys("audience:*");
		return keys.stream().filter(str->str.split(":").length==2)
			.map(key->hashOps.get(key, "money"))
			.filter(key->key!=null)
			.mapToLong(amount->Long.parseLong(amount))
			.reduce(0L, (a,b)->a+b);
	}
