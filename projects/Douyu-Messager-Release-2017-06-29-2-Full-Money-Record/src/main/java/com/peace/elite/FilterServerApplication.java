 ...
 */	
	...
	private String audienceFetch(long uid){
		Map<String, String> audienceProfile = audienceRedisRepository.audienceFetch(uid);
		return "总消费:" + audienceProfile.get("money") + ", 总发言:"+audienceProfile.get("chat");
	}
	...
	public void convertStreamToString(java.io.InputStream is) throws IOException {
		...
		while (true) {
			...
			parsedData.stream().filter(map->{
				...
				//return (143604960+"").equals(map.get("uid"));
				return true;
					...
					if(type !=null){
						...
						switch(type){
							...
							case chatmsg:
								...
								//display = printMessage(chatMessage);
								break;
							case onlinegift: 
							...
							case dgb:
								...
								//}
								display = smallGift.toString() + audienceFetch(smallGift.getUid());
								break;
							case uenter:
